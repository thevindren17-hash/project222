...
def _build_date_context(
    ...
    return (
        ...
        "- ONCE book_appointment has already succeeded earlier in this conversation, that appointment is DONE — "
        "do NOT call book_appointment again for it, no matter what the patient says next (declining notes, saying "
        "'no', going quiet and then saying something vague like 'hey' or 'ok'). Only call book_appointment again if "
        "the patient is CLEARLY asking to book a SEPARATE, ADDITIONAL appointment with its own new date/time/service "
        "— never as a reaction to an unrelated or ambiguous reply. If a reply after a successful booking is unclear, "
        "just ask what they'd like next — do not silently retry the booking.\n"
        ...
        "- Once you have read back the full booking details (service, date, time, name) and the patient replies "
        "confirming them (e.g. 'yes', 'correct', 'okay', 'betul', '是'), you MUST call book_appointment in that exact "
