...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (!(file instanceof Blob)) {
      return NextResponse.json({ error: 'A template header image file is required' }, { status: 400 })
    }
    ...
    outgoing.set('file', file, (file as File).name || 'header.jpg')

    const upstream = await fetch(`${BACKEND.replace(/\/$/, '')}/api/templates/${id}/media`, {
      method: 'POST',
